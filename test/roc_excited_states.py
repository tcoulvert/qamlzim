...
AUGMENT_SIZE = (
    7  # must be an odd number (since augmentation includes original value in middle)
)
    ...
    for i in range(n_classifiers):
        for j in range(scale):
            predictions[i * scale + j] = np.sign(
                predictions_raw[i] + (j - scale // 2) * offset
            ) / (n_classifiers * scale)
    ...
    for i in range(len(train_sizes)):
        ...
        for f in range(n_folds):
            ...
            if f == 1:
                ...
                run = 1
            print("fold", f)
            train_sig = rand_delete(
                remaining_sig, sig_pct * train_size, train_data=True
            )
            train_bkg = rand_delete(
                remaining_bkg, bkg_pct * train_size, train_data=True
            )
            ...
            predictions_test, y_test = create_augmented_data(
                sig[test_sig], bkg[test_bkg]
            )
            predictions_valid, y_valid = create_augmented_data(
                sig[valid_sig], bkg[valid_bkg]
            )
            if mus_filenames is None:
                mus_filename = (
                    "FAILmus04000-7_iter%d_run%d__2022-03-25_15-18-47.npy" % (f, run)
                )
            else:
            ...
            if mus_dir is None:
                mus_destdir = os.path.join(
                    script_path, "../qamlz_for_higgs/mus/2022-03-25_15-18-47/"
                )
            else:
                mus_destdir = mus_dir
            ...
            for p in range(poisson_runs):
                ...
                for w in range(len(excited_weights)):
                    continuous_predictions = ensemble(
                        predictions_test, excited_weights[w]
                    )
                    bkg_rejection, sig_efficiency = auc(
                        continuous_predictions, y_test, test_sig, test_bkg, poisson
                    )
                    interp = interp1d(bkg_rejection, sig_efficiency, kind="cubic")

                    valid_continuous_predictions = ensemble(
                        predictions_valid, excited_weights[w]
                    )
                    valid_bkg_rejection, valid_sig_efficiency = auc(
                        valid_continuous_predictions,
                        y_valid,
                        valid_sig,
                        valid_bkg,
                        valid_poisson,
                    )
                    valid_interp = interp1d(
                        valid_bkg_rejection, valid_sig_efficiency, kind="cubic"
                    )
                ...
                sig_efficiency_max = sig_efficiencies[
                    (sig_efficiency_ind, np.arange(len(bkg_grid)))
                ]
        ...
        mean = np.mean(cts)
        std = np.std(cts)
        auroc_results["data"].append(
            {"train_size": train_size, "mean": mean, "std_dev": std}
        )
