...
sig_X_train, sig_X_test, sig_y_train, sig_y_test = train_test_split(sig_X, sig_y, test_size=0.5, shuffle=True, random_state=0)
bkg_X_train, bkg_X_test, bkg_y_train, bkg_y_test = train_test_split(bkg_X, bkg_y, test_size=0.5, shuffle=True, random_state=0)

X_train, y_train = np.vstack((sig_X_train, bkg_X_train)), np.concatenate((sig_y_train, bkg_y_train))
...
for train_size in train_sizes:
    ...
    env = qamlz.TrainEnv(X_train[rand_train_indeces], y_train[rand_train_indeces], url, token, fidelity=7, dwave_topology='chimera')
    ...
    time.sleep(5)
    roc_main(sig=sig_X_test, bkg=bkg_X_test, mus_dir=mus_dir, mus_filenames=mus_filenames, train_size=train_size)