...
def dwave_connect(config, iter, sampler, bqm, bqm_nx, anneal_time):
    ...
    for g in range(config.ngauges[iter]):
        ...
        unembed_qaresult = dw.embedding.unembed_sampleset(
            qaresult, config.embedding, bqm
        )

        for i in range(len(unembed_qaresult.record.sample)):
            unembed_qaresult.record.sample[i, :] = (
                unembed_qaresult.record.sample[i, :] * a
            )
        qaresults[
            g * config.nread : (g + 1) * config.nread
        ] = unembed_qaresult.record.sample
...
def energies(spins, sigma, qaresults, C_i, C_ij, mu):
    ...
    en_energies += (
        2
        * np.einsum(
            "ij, j", np.einsum("ij, jk", spins, np.triu(C_ij, k=1)), spins[0][:]
        )
        * pow(sigma, 2)
    )
...
#   -> bc majority vote happens earlier, qac has the same structure as non-qac
def uniques_qac(en_energies, energy_fraction, max_states):
    return unique_energies(en_energies, energy_fraction, max_states)


# Picks out the unique energies out of the ones derived from energies (copy method)
def uniques_copy(multi_energies, energy_fraction, max_states):
    """
    TODO:
    """
    multi_uniques = np.zeros(np.size(multi_energies, 0))
    for i in range(np.size(multi_energies, 0)):
        multi_uniques[i] = energies(multi_energies[i], energy_fraction, max_states)
...
def anneal(config, iter, env, mu):
    ...
    if config.encode_vars is None:
        ...
        bqm, bqm_nx, fixed_dict = config.encode_vars(
            h, J, config.fix_vars, config.encoding_depth
        )

    qaresults = dwave_connect(
    ...
    if config.encode_vars is None and config.fix_vars is not None:
        ...
        spins = config.decode_vars(
            qaresults, np.size(h), orig_len, config.fix_vars, fixed_dict=fixed_dict
        )
        en_energies = config.encoded_energies(
            spins, pow(config.zoom_factor, iter), qaresults, env.C_i, env.C_ij, mu
        )
        unique_indices = config.encoded_uniques(
            en_energies, config.energy_fractions[iter], config.max_states[iter]
        )
    ...
    en_energies = energies(
        spins, pow(config.zoom_factor, iter), qaresults, env.C_i, env.C_ij, mu
    )
    unique_indices = unique_energies(
        en_energies, config.energy_fractions[iter], config.max_states[iter]
    )
    excited_states_arr = [spins[unique_indices]]
